...
public class VMTranslator {
    ...
    public void translate() throws Exception {
        ...
        List<String> bootstrapCode = new ArrayList<>(List.of("@256", "D=A", "@SP", "M=D", "@SKIP", "0;JMP"));
        ...
        for (Address a : pushAddresses) {
            ...
            if (a.getSegment().equals("constant") && a.getIndex() < 0){
                label = "PUSH_" + a.getSegment() + "_" + (a.getIndex() * -1) + "_NEG";
            }
            switch (a.getSegment()) {
                case "constant" -> {
                    if (a.getIndex() < 0) {
                        bootstrapCode.addAll(List.of("(" + label + ")", "@" + (a.getIndex() * -1), "D=-A", "@PC_PUSH", "0;JMP"));
                    } else {
        ...
        bootstrapCode.addAll(List.of("// gt: D > 0", "(DO_GT)", "@RETURN_TRUE", "D;JGT", "@RETURN_FALSE", "0;JMP"));
        bootstrapCode.addAll(List.of("// eq: D == 0", "(DO_EQ)", "@RETURN_TRUE", "D;JEQ", "@RETURN_FALSE", "0;JMP"));
        bootstrapCode.addAll(List.of("// lt: D < 0", "(DO_LT)", "@RETURN_TRUE", "D;JLT", "@RETURN_FALSE", "0;JMP"));
        bootstrapCode.addAll(List.of("// Set boolean in D", "(RETURN_TRUE)", "D=-1", "@WRITE_BACK", "0;JMP", "(RETURN_FALSE)", "D=0", "@WRITE_BACK", "0;JMP"));
        bootstrapCode.addAll(List.of("// Collapse stack and return", "(WRITE_BACK)", "@SP", "AM=M-1", "D=M", "@15", "AM=D+M;JMP"));
        ...
        bootstrapCode.addAll(List.of("(PC_2)", "@15", "M=M+1", "AM=M+1;JMP", "(SKIP)"));
        // Add bootstrap code
        ...
        for (String function : reachableFunctions) {
            ...
            for (VMinstruction inst : instructions) {
                ...
                VMTranslator.thread = true;
                List<String> assembly_threaded = new ArrayList<>();
                for (VMinstruction v : inst.unWrap()) {
                    assembly_threaded.addAll(v.decode());
                }

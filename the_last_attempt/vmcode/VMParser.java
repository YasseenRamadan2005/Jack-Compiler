    ...
    private VMinstruction parseLine(String line) {
        ...
        switch (cmd) {
            ...
            case "label" -> {
                ...
                return new LabelInstruction(tokens[1], moduleName, currentFunction);
            }
            case "goto" -> {
                requireLength(tokens, 2, line);
                return new GotoInstruction(tokens[1], moduleName, currentFunction);
            }
            case "if-goto" -> {
                requireLength(tokens, 2, line);
                return new IfGotoInstruction(tokens[1], moduleName, currentFunction);
            }
            case "function" -> {
                requireLength(tokens, 3, line);
